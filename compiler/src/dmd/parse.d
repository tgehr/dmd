  ...
  final:
    ...
    bool isTypeConstructor(Token* t)
    {
        ...
        if (next.value != TOK.leftParenthesis)
            return false;
        if (peekPastParen(next).value == TOK.assign)
        {
            // const (...) = ...
            return false;
        }
    ...
    private AST.Dsymbols* parseDeclarations(bool autodecl, PrefixAttributes!AST* pAttrs, const(char)* comment)
    {
        ...
        if (!autodecl)
        {
            ...
            {
                auto next = peek(&token);
                if (isVariableStorageClass(next.value) || isDeclaration(next, NeedDeclaratorId.mustIfDstyle, TOK.reserved, null))
                {
            ...
            */
            if ((storage_class || udas) && token.value == TOK.identifier && hasOptionalParensThen(peek(&token), TOK.assign) ||
                token.value == TOK.leftParenthesis && peekPastParen(&token).value == TOK.assign)
            {
        ...
        Lagain:
            ...
            switch (token.value)
            {
                ...
                case TOK.const_:
                    ...
                    if (next.value == TOK.leftParenthesis)
                    {
                        TOK after = peekPastParen(next).value;
